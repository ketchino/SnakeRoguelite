...
function renderRelList() {
    ...
    if (!G.relics.length) { var empty = document.createElement("div"); empty.style.cssText = "color:#555;font-size:12px"; empty.textContent = "\uD83D\uDEE1\uFE0F Nessuna reliquia"; wrap.appendChild(empty); }
    else { G.relics.forEach(function (id) {
        var r = RELICS.find(function (p) { return p.id === id; }); if (!r) return;
        ...
        inner.appendChild(nameSpan); inner.appendChild(descSpan); row.appendChild(icon); row.appendChild(inner); wrap.appendChild(row);
    }); }
    return wrap;
}
